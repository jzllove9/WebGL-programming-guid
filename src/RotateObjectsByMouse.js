...
window.onload = function() {
	...
	/*初始化鼠标事件*/
	initMouseEventHandlers(_canvas, globalObj.currentAngle);
...
function initVertexBuffers(gl) {
	...
	if (!initBuffer(gl, colors, 4, gl.FLOAT, "a_Color", "")) return -1;
...
function model_view_matrix(flag) {
	...
	modelViewMatrix.rotate(globalObj.currentAngle[0], 1.0, 0.0, 0.0);
	modelViewMatrix.rotate(globalObj.currentAngle[1], 0.0, 1.0, 0.0);
...
/*鼠标事件响应*/
function initMouseEventHandlers(canvas, currentAngle) {
	let dragging = false;
	let lastX = -1,
		lastY = -1;
	...
	canvas.onmousedown = function(e) {
		...
		if (rect.left <= x && x < rect.right && rect.top <= x && x < rect.bottom) {
			lastX = x;
			lastY = y;
			dragging = true;
		}
	...
	canvas.onmousemove = function(e) {
		...
		if (dragging) {
			...
			globalObj.currentAngle[0] = Math.max(Math.min(globalObj.currentAngle[0] + dy, 90), -90);
			globalObj.currentAngle[1] = currentAngle[1] + dx;
			draw();
		}
